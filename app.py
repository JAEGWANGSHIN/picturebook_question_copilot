...
def save_result(key: str, label: str, title: str, result: Any) -> None:
    st.session_state.results[key] = {"label": label, "title": title or "그림책 질문수업", "result": result}
...
with tabs[1]:
    ...
    with col1:
        grade = st.selectbox("학년 선택", ["초등학교 1학년", "초등학교 2학년", "초등학교 3학년", "초등학교 4학년", "초등학교 5학년", "초등학교 6학년"], index=2)
        subject = st.text_input("교과 입력", value="국어, 도덕, 창의적 체험활동")
        achievement = st.text_area("성취기준 입력", value="인물의 마음을 짐작하고 자신의 생각을 친구와 나눈다.")
    with col2:
        topic = st.text_input("수업 주제 입력", value="다름을 존중하기")
        class_context = st.text_area("학급 상황 입력", value="친구의 외모나 말투를 놀리는 일이 있다.")
        lesson_minutes = st.slider("수업 시간 선택", 20, 80, 40, 5)
        activity_style = st.multiselect("원하는 활동 방식 선택", ["짝 토론", "모둠 토론", "생각 쓰기", "마음성장노트", "역할극", "활동지", "카드 만들기"], default=["짝 토론", "생각 쓰기"])

    if st.button("그림책 추천하기", type="primary"):
...
with tabs[2]:
    ...
    uploaded = st.file_uploader("PDF/JPG/PNG/TXT 업로드", type=["pdf", "jpg", "jpeg", "png", "txt", "md"])
    scan_title = st.text_input("그림책 제목 입력", value="")
    pasted_ocr = st.text_area("OCR 텍스트 붙여넣기", height=220, placeholder="vFlat SCAN에서 추출한 OCR 텍스트를 여기에 붙여넣으세요.")

    if st.button("자료화하기", type="primary"):
...
with tabs[3]:
    ...
    selected = st.selectbox("그림책 선택 또는 직접 입력", titles)
    if selected == "직접 입력":
        q_title = st.text_input("그림책 제목", value="내 귀는 짝짝이")
    ...
    q_summary = st.text_area("그림책 요약 또는 교사의 수업 구상", value=default_summary, height=140)
    q_grade = st.selectbox("학년 선택", ["초등학교 1학년", "초등학교 2학년", "초등학교 3학년", "초등학교 4학년", "초등학교 5학년", "초등학교 6학년"], index=2, key="q_grade")
    q_topic = st.text_input("수업 주제", value="다름과 자기수용")

    if st.button("질문 10개 생성", type="primary"):
...
with tabs[4]:
    ...
    col1, col2 = st.columns(2)
    with col1:
        lp_title = st.text_input("그림책 제목", value=st.session_state.last_title if st.session_state.last_title != "그림책 질문수업" else "내 귀는 짝짝이")
        lp_grade = st.text_input("학년", value="초등학교 3학년")
        lp_subject = st.text_input("교과", value="국어, 도덕")
        lp_minutes = st.slider("수업 시간", 20, 80, 40, 5, key="lp_minutes")
    with col2:
        lp_idea = st.text_area("수업 구상", value="다르다는 것은 고쳐야 할 문제가 아니라 존중하고 이해할 수 있는 특징임을 질문과 토론으로 나눈다.", height=120)
        existing_questions = ""
        if "question_cards" in st.session_state.results:
            existing_questions = "\n".join([x.get("질문", "") for x in st.session_state.results["question_cards"]["result"]])
        lp_questions = st.text_area("질문 목록", value=existing_questions, height=180)

    if st.button("지도안 생성", type="primary"):
...
우리 반에서 서로 다름을 인정하려면 무엇을 해야 할까?"""
    student_qs = st.text_area("학생 질문 목록 붙여넣기", value=sample_questions, height=220)

    if st.button("질문 분류 및 수업화", type="primary"):
...
with tabs[6]:
    ...
    if not st.session_state.results:
        ...
        selected_key = st.selectbox("현재 생성 결과 선택", list(labels.keys()), format_func=lambda k: labels[k])
        selected_result = st.session_state.results[selected_key]
        md_text = result_to_markdown(selected_result["title"], selected_result["label"], selected_result["result"])

        st.text_area("복사하기용 텍스트", value=md_text, height=420)
        ...
        st.download_button("Markdown 다운로드", data=md_text.encode("utf-8"), file_name=md_filename, mime="text/markdown")
        try:
            docx_bytes = markdown_to_docx_bytes(md_text)
            st.download_button("DOCX 다운로드", data=docx_bytes, file_name=docx_filename, mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
